...
class $modify(GrdDemonFilterSelectLayer, DemonFilterSelectLayer) {
    ...
    bool init() {
        ...
        auto s9spr = static_cast<CCScale9Sprite*>(m_mainLayer->getChildByID("background"));
        auto label = static_cast<CCLabelBMFont*>(m_mainLayer->getChildByID("demon-filter-text"));


        // Find OK Button
        auto okButton = static_cast<CCMenuItemSpriteExtra*>(m_buttonMenu->getChildByID("close-button"));
        ...
        label->setScale(0.8f);
        m_buttonMenu->setPositionY(newcsize.height * 0.82f);
        ...
        newMenu->setPosition({m_mainLayer->getContentSize().width / 2 - 3, m_mainLayer->getContentSize().height * 0.38f});
        newMenu->setLayout(RowLayout::create()
        ->setGap(-1.f)
        ->setAxisAlignment(AxisAlignment::Center));
        
        auto instantSearch = CCLabelBMFont::create("Instant Search", "bigFont.fnt");
        instantSearch->setPosition({label->getPositionX(), m_mainLayer->getContentSize().height * 0.52f});
        instantSearch->setScale(0.8f);
        m_mainLayer->addChild(instantSearch);
